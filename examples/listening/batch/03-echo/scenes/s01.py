...
class Film(Stage):
    def construct(self):
        ...
        self.at('Move the listener');self.play(FadeOut(VGroup(out,back,a,b)),source.animate.set_value(.1),run_time=1.5);self.play(ReplacementTransform(label,self.label('0.5 s round trip',[0,4.7,0],role='claim')),ReplacementTransform(distance,self.label('distance ≈ 86 m',[0,-3.3,0],role='claim')),run_time=.5)

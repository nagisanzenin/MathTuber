...
POLY=[(0,0),(4,0),(4,2),(2,3),(0,3)]
NEW=[(0,0),(4,0),(4,2),(2,4),(0,3)]
def point(x,y):return np.array([(x-2)*1.05,(y-1.5)*1.05+.5,0])
def category(x,y,vertices):
    for (a,b),(c,d) in zip(vertices,vertices[1:]+vertices[:1]):
        if (x-a)*(d-b)==(y-b)*(c-a) and min(a,c)<=x<=max(a,c) and min(b,d)<=y<=max(b,d):return 'boundary'
    inside=False
    for (a,b),(c,d) in zip(vertices,vertices[1:]+vertices[:1]):
        if (b>y)!=(d>y) and x<(c-a)*(y-b)/(d-b)+a:inside=not inside
    return 'inside' if inside else 'outside'
def lattice(scene,vertices):
    shape=Polygon(*[point(*v) for v in vertices],fill_color=scene.palette['primary'],fill_opacity=.13,stroke_color=scene.palette['primary'],stroke_width=4)
    grid=VGroup(*[Dot(point(x,y),radius=.043,color=scene.palette['muted']) for x in range(5) for y in range(5)])
    interior=VGroup(*[Dot(point(x,y),radius=.11,color=scene.palette['primary']) for x in range(5) for y in range(5) if category(x,y,vertices)=='inside'])
    boundary=VGroup(*[Circle(radius=.12,color=scene.palette['secondary'],stroke_width=3).move_to(point(x,y)) for x in range(5) for y in range(5) if category(x,y,vertices)=='boundary'])
    return shape,grid,interior,boundary

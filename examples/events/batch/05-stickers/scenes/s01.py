...
class Film(Stage):
    def construct(self):
        ...
        self.at('Our first sticker');self.play(*[album[i][1].animate.set_fill(self.palette['surface'],1) for i in range(6)],run_time=.5);self.play(album[0][1].animate.set_fill(self.palette['primary'],1),run_time=.6)
        self.at('With three spaces');self.play(*[album[i][1].animate.set_fill(self.palette['primary' if i<3 else 'surface'],1) for i in range(6)],run_time=.7);mark=VGroup(*[SurroundingRectangle(album[i],color=self.palette['secondary'],buff=.07) for i in range(3,6)]);self.play(Create(mark),run_time=.6)
        self.at('average wait is two');wait=self.label('3 of 6 help → average wait 2',[0,-1.4,0],'primary');self.add(wait)
        self.at('With just one space');self.play(FadeOut(mark),FadeOut(wait),*[album[i][1].animate.set_fill(self.palette['primary' if i<5 else 'surface'],1) for i in range(6)],run_time=.7);mark=SurroundingRectangle(album[5],color=self.palette['secondary'],buff=.07);self.add(mark)

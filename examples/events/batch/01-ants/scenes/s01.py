...
class Film(Stage):
    def construct(self):
        ...
        self.at('Our ants are points');self.play(FadeOut(a),FadeOut(b),run_time=.4);starts=[1,3,4,7,9];dirs=[1,1,-1,-1,1];xx=lambda x:-3+.6*x;ants=VGroup(*[ant(xx(x),1.5,'primary') for x in starts]);arrows=VGroup(*[Arrow([xx(x),2.05,0],[xx(x)+.4*v,2.05,0],buff=0,color=self.palette['ink'],stroke_width=3) for x,v in zip(starts,dirs)]);self.add(ants,arrows,self.label('10 m stick • speed 1 m/s',[0,.55,0],'ink','detail'))
        ...
        for i,(x,v,t) in enumerate(zip(starts,dirs,[9,7,4,7,1])):
            y=3-i*1.2;self.add(self.line([-3,y,0],[3,y,0],'surface',8),Dot([xx(x),y,0],radius=.1,color=self.palette['primary']),Arrow([xx(x),y,0],[xx(10 if v>0 else 0),y,0],buff=.04,color=self.palette['primary'],stroke_width=4),self.label(str(t)+' s',[2.7 if v<0 else -2.7,y+.32,0],'ink','detail'))

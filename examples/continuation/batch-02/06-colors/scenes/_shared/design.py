...
class Stage(WorkshopScene):
    ...
    def label(self,text,pos,color='ink',role='label'):
        return self.lettering(text,role,'ink' if color=='accent' else color,max_width=6.0).move_to(pos)
    ...
    def card(self,s,p,color='primary',w=1.0,h=.75):
        rect=RoundedRectangle(width=w,height=h,corner_radius=.10,stroke_color=self.palette[color],stroke_width=2,fill_color=self.palette[color],fill_opacity=.10).move_to(p)
        return VGroup(rect,self.label(s,p,'ink'))

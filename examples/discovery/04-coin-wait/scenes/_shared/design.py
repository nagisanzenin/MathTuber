...
def cards(n=8):
    colors=[interpolate_color(BLUE,PURPLE,i/max(n-1,1)) for i in range(n)]
...
def shuffle(scene,deck,order):
    ...
    scene.play(*[deck[k].animate.move_to([(j-(n-1)/2)*.78,0,0]) for j,k in enumerate(new)],run_time=1.05,path_arc=.7)
    return new

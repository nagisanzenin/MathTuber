...
from mathtuber.media import assembly_fingerprint
...
def main():
    ...
    duration = export["metadata"]["duration"]
    times = ending_samples(duration, args.seconds, args.samples, project.data.get("format", {}).get("fps", 30))
    ...
    for i, timestamp in enumerate(times):
        ...
        subprocess.run(["ffmpeg", "-v", "error", "-y", "-ss", str(timestamp), "-i", export["absolute_path"], "-frames:v", "1", "-vf", "scale=360:640", str(frame)], check=True)
    ...
    atomic_json(out/"evidence.json", dict(export_sha256=export["sha256"], frames=records, sheet_sha256=file_hash(path), coverage="Sampled final section only; not continuous audiovisual viewing or a review verdict."))
    print(path)

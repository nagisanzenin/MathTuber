...
A direct tracker reset followed by a static wait can leave an earlier diagram on screen. Use an explicit transition (or deliberately force frame updates) before replaying a comparison. Inspect the actual reference and changed states during every sound window, including repeat demonstrations; a correct score does not establish a correct picture. Use `replace_label` for numeral changes instead of deforming text with a shape morph.

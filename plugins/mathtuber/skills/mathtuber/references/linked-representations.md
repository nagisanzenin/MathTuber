...
When a passage only narrates conclusions over a diagram, consider one meaningful comparison: establish a common situation, change a relevant condition and preserve the reference that makes its consequence interpretable. Do not present two unfamiliar representations simultaneously just to make the layout busier. A short invitation to predict is optional and must resolve visibly; avoid countdowns or engagement barriers. Inspect the entire comparison before extending it across a batch. These are editorial hypotheses, not validated retention rules; the repository's `docs/research/CONTRAST-WITHOUT-OVERLOAD.md` records the studies and their limits.

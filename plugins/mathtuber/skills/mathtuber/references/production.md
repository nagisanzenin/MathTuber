...
The optional `components.NarratedScene` gives `target_duration`, `cue(fraction)` and `finish()`. A bound profile supplies its background, legacy color aliases and heading font; the dark theme remains only for unbound legacy work. Prefer `WorkshopScene` for profile lettering and layout helpers. Fractions are coarse timing aids. For precise speech synchronization use the measured word timestamps in the audio sidecar and explicit waits. Design the geometry each explanation needs. Profile-bound scenes must inherit `NarratedScene` or `WorkshopScene`; this does not restrict their mathematical or visual vocabulary.
...
For independent speech inspection, install the optional local recognizer once with `python3 <root>/scripts/setup.py --with-review`, then run `python3 <root>/scripts/audio_review.py --project <path> --final`. The command selects the shared media Python itself. It checks every current narration artifact, transcribes sources with faster-whisper `small.en`, compares the complete text, and measures the final stereo soundtrack. Inspect `reviews/audio-inspection.json`, including every discrepancy and the current source/export hashes. It reuses a matching report when inputs and options are unchanged. First ASR use downloads model weights; subsequent inference is local. Other languages need a suitable multilingual `--model` and `--language`.

`--no-asr` produces signal measurements with transcription explicitly unavailable. Omitting `--final` permits source inspection before assembly. Neither mode records acceptance. ASR segment times are approximate recognition timestamps, not forced alignment; speech wording differences can be recognizer errors. Number spelling differences are common, so a low similarity ratio alone must not trigger a resynthesis. Silence, clipping and RMS measurements cannot judge pleasant delivery or whether sound supports the explanation. Final soundtrack measurement does not transcribe a separate soundtrack or establish a good mix.
